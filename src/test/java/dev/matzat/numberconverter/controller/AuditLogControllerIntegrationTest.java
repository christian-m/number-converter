...
public class AuditLogControllerIntegrationTest extends SpringBootTestBase {
    ...
    public void testAuditLogPageOne() {
        webClient.get()
    ...
    @FlywayTest
    @DisplayName("GIVEN four audit log entries in the database WHEN a audit log page 1 is requested THEN the list with two entries is returned")
    public void testAuditLogPageTwo() {
        webClient.get()
            .uri("/audit-log?page=1&size=2&sort=created")
            .exchange()
            .expectStatus().isOk()
            .expectBody()
            .jsonPath("$.content[0].timestamp").isEqualTo(Instant.ofEpochMilli(1734104603000L))
            .jsonPath("$.content[0].input").isEqualTo("input 3")
            .jsonPath("$.content[0].output").isEqualTo("output 3")
            .jsonPath("$.content[0].statusCode").isEqualTo("422 UNPROCESSABLE_ENTITY")
            .jsonPath("$.content[0].success").isEqualTo(false)
            .jsonPath("$.content[1].timestamp").isEqualTo(Instant.ofEpochMilli(1734104663000L))
            .jsonPath("$.content[1].input").isEqualTo("input 4")
            .jsonPath("$.content[1].output").isEqualTo("output 4")
            .jsonPath("$.content[1].statusCode").isEqualTo("422 UNPROCESSABLE_ENTITY")
            .jsonPath("$.content[1].success").isEqualTo(false)
            .jsonPath("$.total").isEqualTo(4)
            .jsonPath("$.pageable.pageNumber").isEqualTo(1)
            .jsonPath("$.pageable.pageSize").isEqualTo(2)
            .jsonPath("$.pageable.offset").isEqualTo(2);
    }
}
